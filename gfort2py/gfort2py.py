...
class fFort(object):
        ...
        with open(self._fpy, 'rb') as f:
            ...
            if self.version == version.__version__:
                ...
                if self._mod_data["checksum"] != pm.hashFile(ffile) or rerun:
                    ...
                    self._var = pickle.load(f)
                    self._param = pickle.load(f)
                    self._func = pickle.load(f)
                    self._dt_defs = pickle.load(f)
                    self._func_ptrs = pickle.load(f)
            else:
                self._rerun(ffile)
    ...
    def _init_var(self, name):
        ...
        self._var[name] = fVar(self._lib, self._var[name],const=False, name=name)

    def _init_param(self, name):
        if hasattr(self._param[name],'_init'):
            return
        
        self._param[name] = fVar(self._lib, self._param[name],const=True, name=name)

    def _init_func(self, name):
        if hasattr(self._func[name],'_init'):
            return

        self._func[name] = fFunc(self._lib, self._func[name], name)
    ...
    def _init_struct(self, name):
        ...
        # access through cvar
        self._var[name] = fVar(self._lib, self._structs[name], name)
    ...
    def __getattr__(self, name):
        if name in self.__dict__:
            ...
            if '_param' in self.__dict__ and name in self._param:
                self._init_param(name)
                return self._param[name]

            elif '_func' in self.__dict__ and name in self._func:
    ...
    def __setattr__(self, name, value):
        if name in self.__dict__:
            ...
            if '_var'  in self.__dict__ and name in self._var:
                ...
                return
            elif '_param'  in self.__dict__ and name in self._param:
                self._init_param(name)
                self._param[name].set(value)
                return
